...
// 🎯 新增：导入领域服务
import { 
  initializeLiteratureDomain, 
  literatureService as domainLiteratureService,
  taskManagementService,
  type LiteratureOperationResult,
  type AddLiteratureOptions
} from '../libs/literature';
...
export const useLibraryStore = create<LibraryState & LibraryActions>((set, get) => ({
  ...
  initialize: async () => {
    try {
      ...
      // 🎯 初始化领域层服务和依赖关系
      console.log('🔧 [LibraryStore] Initializing literature domain services...');
      initializeLiteratureDomain();
